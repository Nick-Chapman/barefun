...
let eq_list =
  fix (fun[] eq_list eq k ->
    ...
    k v70) in
let eq_char_list =
  (fun[eq_char,eq_list] a k ->
    let v73 =
      (fun[a,eq_char,eq_list] b k ->
...
let put_string_newline =
  (fun[newline,put_string] s k ->
    ...
    put_string s k) in
let read_line =
  (fun[<=,=,>,chr,cons,eq_char,get_char,newline,ord,put_char,reverse] _ k ->
    let loop =
