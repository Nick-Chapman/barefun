...
let put_int =
  ...
  (fun i ->
    match ((< i) 0) with
    | true -> (put_chars ((:: '-') (chars_of_int ((- 0) i))))
    | false -> (put_chars (chars_of_int i)))) in
type _ = VALUE
let deVal =
  (fun v ->
    match v with
    | VALUE(i) -> i) in
let vadd = (fun v1 -> (fun v2 -> VALUE(((+ (deVal v1)) (deVal v2))))) in
let vdec = (fun v -> VALUE(((- (deVal v)) 1))) in
type _ = LOAD_IMMEDIATE | STORE_LOCAL | LOAD_LOCAL | ADD | DEC | PRINT | PRINTI | JMPNZ | HALT
let program =
  let result = 0 in
  let loopc = 1 in
  Cons(LOAD_IMMEDIATE(VALUE(0)), Cons(STORE_LOCAL(result), Cons(LOAD_IMMEDIATE(VALUE(goal)), Cons(STORE_LOCAL(loopc), Cons(ADD(result, loopc), Cons(STORE_LOCAL(result), Cons(LOAD_LOCAL(loopc), Cons(DEC, Cons(STORE_LOCAL(loopc), Cons(JMPNZ(4), Cons(PRINT("(Ocaml)Result: "), Cons(LOAD_LOCAL(result), Cons(PRINTI, Cons(PRINT("\n"), Cons(HALT, Nil))))))))))))))) in
let drop =
  fix (fun drop i ->
    (fun xs ->
      match ((= i) 0) with
    ...
    let local_at =
      ...
      (fun i ->
        ...
        | false ->
          match ((= i) 1) with
          | true -> (! local1)
          | false -> (crash "local_at"))) in
    let acc = (ref zero) in
    let loop =
      fix (fun loop ops ->
        match ops with
        | Nil -> (crash "run out of instructions")
        | Cons(op,ops) ->
          match op with
          | LOAD_IMMEDIATE(v) ->
            let _ = ((:= acc) v) in
